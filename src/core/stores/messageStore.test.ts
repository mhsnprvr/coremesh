...
describe("useMessageStore", () => {
  ...
  describe("saveMessage", () => {
    it("should save a direct message with correct structure", () => {
      ...
      expect(
        state.messages
          .direct[myNodeNum][otherNodeNum1][directMessageToOther1.messageId],
    ...
    it("should save multiple messages correctly", () => {
      ...
      expect(
        state.messages.direct[myNodeNum]?.[otherNodeNum1]
          ?.[directMessageToOther1.messageId],
      ).toEqual(directMessageToOther1);
      // Direct msg 2 (other1 -> me)
      expect(
        state.messages.direct[otherNodeNum1]?.[myNodeNum]
          ?.[directMessageFromOther1.messageId],
      ).toEqual(directMessageFromOther1);
      // Broadcast msg 1
      expect(
        state.messages.broadcast[broadcastChannel]
  ...
  describe("setMessageState", () => {
    ...
    it("should update state for a direct message sent BY ME", () => {
      ...
      const message = useMessageStore.getState().messages.direct[myNodeNum]
        ?.[otherNodeNum1]?.[directMessageToOther1.messageId];
    ...
    it("should update state for a direct message received FROM OTHER", () => {
      ...
      const message = useMessageStore.getState().messages.direct[otherNodeNum1]
        ?.[myNodeNum]?.[directMessageFromOther1.messageId];
    ...
    it("should update state for a broadcast message", () => {
      ...
      const message = useMessageStore.getState().messages
        .broadcast[broadcastChannel]?.[broadcastMessage1.messageId];
      expect(message?.state).toBe(MessageState.Ack);
    });
    ...
    it("should warn if message is not found", () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("Message not found for state update"),
      );
      warnSpy.mockRestore();
    });
  });
  ...
  describe("clearMessageByMessageId", () => {
    ...
    it("should delete a specific direct message (sent by me)", () => {
      ...
      const state = useMessageStore.getState();
      expect(
        state.messages.direct[myNodeNum]?.[otherNodeNum1]?.[messageIdToDelete],
      ).toBeUndefined();
      expect(state.messages.direct[myNodeNum]?.[otherNodeNum1]?.[1011])
        .toBeDefined();
      expect(
        state.messages.direct[otherNodeNum1]?.[myNodeNum]
          ?.[directMessageFromOther1.messageId],
    ...
    it("should delete a specific direct message (sent by other)", () => {
      ...
      const state = useMessageStore.getState();
      expect(
        state.messages.direct[otherNodeNum1]?.[myNodeNum]?.[messageIdToDelete],
      ).toBeUndefined();
      expect(
        state.messages.direct[myNodeNum]?.[otherNodeNum1]
          ?.[directMessageToOther1.messageId],
    ...
    it("should delete a specific broadcast message", () => {
      ...
      const state = useMessageStore.getState();
      expect(state.messages.broadcast[broadcastChannel]?.[messageIdToDelete])
        .toBeUndefined();
    });
    ...
    it("should clean up empty to/from/channel objects", () => {
      ...
      expect(
        useMessageStore.getState().messages.direct[otherNodeNum1]?.[myNodeNum],
      ).toBeUndefined(); // Recipient level removed
      expect(useMessageStore.getState().messages.direct[otherNodeNum1])
        .toBeUndefined(); // Sender level removed
      ...
      expect(useMessageStore.getState().messages.broadcast[broadcastChannel])
        .toBeUndefined(); // Channel level removed
    });
  });
  ...
  describe("deleteAllMessages", () => {
    it("should clear all direct and broadcast messages", () => {
      useMessageStore.getState().saveMessage(directMessageToOther1);
      useMessageStore.getState().saveMessage(broadcastMessage1);
      expect(Object.keys(useMessageStore.getState().messages.direct).length)
        .toBeGreaterThan(0);
      expect(Object.keys(useMessageStore.getState().messages.broadcast).length)
        .toBeGreaterThan(0);
